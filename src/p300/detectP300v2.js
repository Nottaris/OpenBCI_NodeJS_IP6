...
var nrOfCommands = Object.keys(vppx).length;    // aka 5
var baselineVolts = [];


function detectP300(volts, command, time) {

    //skip init phase after baseline is ready
    if (init) {
        if(counter===0){
    ...
    //if subBase subtract baseline from values


    //if init is over
    if(!init){
        //if baselineReady (init over) send 5 cycles to python for filter and detect
